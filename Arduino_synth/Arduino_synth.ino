...
// Software I2C: clock pin = 10, data pin = 11
U8G2_SSD1306_128X64_NONAME_F_SW_I2C u8g2(U8G2_R0, /* clock=*/ 10, /* data=*/ 11, /* reset=*/ U8X8_PIN_NONE);
...
uint8_t wave[WAVE_POINTS];
uint8_t waveIndex = 0;
uint8_t scopeDivider = 0;

// To retrigger the grains when Gate first gets pulled HIGH, we need to know if the sound has been just trailing off.
boolean triggerGate;

// Map Analogue channels
#define SYNC_CONTROL         14  // BPM or pitch, depending on how high you turn it. 
                                  // To control pitch with CV while using a tone-frequency Sync value, plug in here.

#define GRAIN_FREQ_CONTROL   15  // Frequency of grain 1
#define GRAIN_DECAY_CONTROL  16  // Decay rate of grain 1

#define GRAIN2_FREQ_CONTROL  17  // Frequency of grain 2
#define GRAIN2_DECAY_CONTROL 18  // Decay rate of grain 2
...
void loop() {
  ...
  */
  ...
  if(millis() - lastDraw > 120) {   // ~120 FPS
    drawWave();
    lastDraw = millis();
  }
}
...
void drawWave() {
  ...
  uint8_t index = waveIndex;

  for(int x=0; x<WAVE_POINTS-1; x++) {

    uint8_t s1 = wave[(index + x) % WAVE_POINTS];
    uint8_t s2 = wave[(index + x + 1) % WAVE_POINTS];

    // Map 0-255 waveform to 0-63 OLED pixels, and invert Y axis
    int y1 = map(s1, 0, 255, 44, 22);
    int y2 = map(s2, 0, 255, 44, 22);

    // Clamp to avoid drawing outside the screen
    y1 = constrain(y1, 22, 44);
    y2 = constrain(y2, 22, 44);
...
SIGNAL(PWM_INTERRUPT) {
  ...
  scopeDivider++;
  if(scopeDivider >= 32) {   // capture about ~1kHz for display
    scopeDivider = 0;

    wave[waveIndex] = output;
    waveIndex++;
